...
// NEW: Prepare Calendar Data (Color-coded by Mood)
  const markedDates = useMemo(() => {
    const marks: Record<string, any> = {};
    
    // Simple internal map - or import from your constants file
    const getMoodColor = (mood?: string) => {
      switch (mood?.toLowerCase()) {
        case 'happy': return '#F59E0B'; // Amber
        case 'calm': return '#10B981';  // Emerald
        case 'sad': return '#64748B';   // Slate
        case 'stressed': return '#EF4444'; // Red
        case 'energetic': return '#F97316'; // Orange
        case 'grateful': return '#8B5CF6'; // Violet
        default: return isDark ? '#818CF8' : '#6366F1'; // Default Indigo
      }
    };
    ...
    entries.forEach(entry => {
      marks[entry.date] = {
        marked: true,
        dotColor: getMoodColor(entry.moodTag?.value), 
      };
    });
    return marks;
  }, [entries, isDark]);
...
const handleRefresh = async () => {
    setRefreshing(true);
    try {
      // Trigger the Cloud Sync action we just created
...
return result;
  ...
  // NEW: Group entries by "Month Year"
  const sections = useMemo(() => {
    const groups: Record<string, JournalEntry[]> = {};
    filteredEntries.forEach(entry => {
      // Create a key like "December 2025"
...
<SectionList
                ...
                initialNumToRender={10}      // Render just enough to fill screen initially
                maxToRenderPerBatch={10}     // Render 10 more items per scroll batch
                windowSize={5}               // Reduce render window (default is 21) to save memory
...
ListEmptyComponent={
  <EmptyState
    icon={List}
    title={selectedMood === 'all' ? 'No Entries Yet' : 'No Matches Found'}
    message={selectedMood === 'all' 
      ? 'Your story begins with a single entry. Take a mindful minute today.' 
      : `We couldn't find any entries tagged with "${selectedMood}".`
    }
    actionLabel={selectedMood === 'all' && !searchText ? "Write New Entry" : undefined}
    onAction={
      selectedMood === 'all' && !searchText 
        ? () => navigation.navigate('Write', { date: new Date().toISOString().split('T')[0] })
        : undefined
    }
  />
}
