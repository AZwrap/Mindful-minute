...
// 🟢 NEW: Instant Test (DISARMED)
export async function runNotificationTest() {
  console.log("🛑 DEBUG: Test function called, but notification is DISABLED.");
  alert("Test function executed. Notification generation is OFF.");

  /* // DISABLED TO STOP PHANTOM NOTIFICATIONS
  const { status } = await Notifications.getPermissionsAsync();
  if (status !== 'granted') {
    // ...
    return;
