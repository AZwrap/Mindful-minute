...
export default function EntryDetailScreen({ route }) {
  ...
  const entry = useEntries((s) => s.getByDate(date));
  ...
  const exportEntry = async (entry, date) => {
    try {
    ...
    >
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <Text style={[styles.date, { color: textSub }]}>{formatDate(date)}</Text>
<Text style={[styles.time, { color: textSub }]}>{formatTime(entry)}</Text>
          ...
          {entry.moodTag?.value && (
            <View style={styles.moodSection}>
              ...
              <View style={[styles.moodTag, { backgroundColor: isDark ? 'rgba(99,102,241,0.15)' : 'rgba(99,102,241,0.08)' }]}>
          ...
          <PremiumPressable
            onPress={() => exportEntry(entry, date)}
            haptic="light"
            style={[
              styles.exportBtn,
              { 
