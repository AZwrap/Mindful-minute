...
import { useJournalTimer } from '../hooks/useJournalTimer';
...
{/* GRATITUDE SECTION */}
          <GratitudeSection 
            isVisible={!!(gratitudeModeEnabled || currentPrompt?.text?.toLowerCase().includes('grateful') || currentPrompt?.text?.toLowerCase().includes('thankful'))}
            entries={gratitudeEntries}
            onEntriesChange={setGratitudeEntries}
            onExpand={(y) => setSectionPositions((prev) => ({ ...prev, gratitude: y }))}
          />
...
timerContainer: { justifyContent: 'center', minHeight: 10, marginbottom: 8 }, // Removed fixed height suggestionsContainer: { flex: 1, marginRight: 12 },
