...
const continueToMood = async () => {
  ...
  upsert({ 
    ...
    createdAt: new Date().toISOString(),
    isComplete: false,
    isGratitude: isGratitudeMode // Add this line
  });
      ...
      {prompt?.explanation && (
        ...
        <Animated.View style={[
          ...
          { 
            ...
            borderRadius: 14,
            borderColor: isGratitudeMode ? '#22C55E' : animatedInputStyle.borderColor, // Override border color for gratitude
          }
        ]}>
          <TextInput
            ref={inputRef}
            ...
            placeholder={isGratitudeMode ? "What are you grateful for today…" : "Take a minute to breathe and write…"}
            placeholderTextColor={isGratitudeMode ? '#22C55E' : placeholderCol}
            multiline
            style={[
              styles.input,
              {
...
onPress={() => {
  ...
  if (hapticsEnabled) {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
          ...
          {/* Gratitude Mode Toggle */}
          <PremiumPressable
            onPress={() => {
              if (hapticsEnabled) Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              setIsGratitudeMode(!isGratitudeMode);
            }}
            haptic="light"
            style={[
              styles.gratitudeToggle,
              { 
                backgroundColor: isGratitudeMode 
                  ? (isDark ? 'rgba(34, 197, 94, 0.2)' : 'rgba(34, 197, 94, 0.1)')
                  : 'transparent',
                borderColor: isGratitudeMode ? '#22C55E' : borderColor
              }
            ]}
          >
            <Text style={[
              styles.gratitudeToggleText,
              { color: isGratitudeMode ? '#22C55E' : textSub }
            ]}>
              🙏 {isGratitudeMode ? 'Gratitude Mode' : 'Gratitude'}
            </Text>
          </PremiumPressable>
