...
{/* BENTO GRID */}
              {(!writingPatterns || writingPatterns.totalEntries === 0) ? (
                 /* EMPTY STATE */
                <View style={[styles.bentoCard, { backgroundColor: palette.card, borderStyle: 'dashed', borderWidth: 2, borderColor: palette.border, alignItems: 'center', padding: 32 }]}>
                   <View style={{ opacity: 0.5, marginBottom: 16 }}>
                      <BarChartIcon size={64} color={palette.accent} />
                   </View>
                   <Text style={{ fontSize: 18, fontWeight: '700', color: textMain, marginBottom: 8 }}>Unlock Your Insights</Text>
                   <Text style={{ fontSize: 14, color: textSub, textAlign: 'center', lineHeight: 20 }}>
                      Write at least one entry to start seeing your streaks, writing patterns, and mood trends here.
                   </Text>
                </View>
              ) : (
