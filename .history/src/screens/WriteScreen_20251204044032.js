...
export default function WriteScreen({ navigation, route }) {
  ...
  async function playSound() {
    if (sound) { await sound.stopAsync(); setIsPlaying(false); return; }
    try {
...
const currentGradient = gradients[currentTheme] || gradients.light;
  const promptColor = palette.text; // <--- Changed to main text color (Black/White)
...
{/* ACTIONS */}
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 16 }}>
            {/* PAUSE BUTTON: Used palette.subtleText for higher visibility border */}
