...
import { Search, X, Trash2 } from 'lucide-react-native';import { SafeAreaView } from 'react-native-safe-area-context';
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          style: "destructive",
          onPress: () => {
            setDeletedEntries(prev => ({ ...prev, [entry.date]: true }));
            const { removeEntry } = useEntriesStore.getState();
            // Note: check if your store uses 'removeEntry' or 'deleteEntry'
            if (removeEntry) removeEntry(entry.date);
            else useEntriesStore.getState().deleteEntry(entry.date);
          }
        }
      ]
    );
...
deleteButton: {
    ...
    justifyContent: 'center',
    alignItems: 'center',
    width: 60, // Specific button width
  ...
  rightActionContainer: {
    width: 80, // Matches the swipe threshold usually
    marginVertical: 6, // Matches entry card margin
    marginRight: 20, // Matches screen padding
    justifyContent: 'center',
    alignItems: 'flex-end',
  },
  deleteTouchArea: {
