...
const SwipeableEntry = React.memo(({ 
  entry, 
  onDelete, 
  onPress,
  isDark,
  ...
  return (
    <Swipeable
      ...
      containerStyle={[styles.swipeableContainer, { height: itemHeight || 'auto' }]}
    >
      <View style={styles.row} onLayout={onLayout}>
        <Pressable
          onPress={onPress}
          style={[styles.entryItem, { borderColor }]}
        >
...
export default function HistoryScreen({ navigation }) {
  ...
  // Filter entries based on selected mood, search, and exclude deleted
  const filteredEntries = useMemo(() => {
    ...
    let filtered = completeEntries;
    
    // Mood filter
    if (selectedMood !== 'all') {
      if (selectedMood === 'custom') {
        filtered = filtered.filter(entry => entry.moodTag?.type === 'custom');
      } else {
        filtered = filtered.filter(entry => entry.moodTag?.value === selectedMood);
      }
    }
            ...
            <View style={[styles.searchContainer, { borderColor }]}>
              ...
              {searchText ? (
                <Pressable onPress={() => setSearchText('')}>
                  <Text style={[styles.clearSearch, { color: textSub }]}>✕</Text>
                </Pressable>
              ) : null}
            </View>
            ...
            ListEmptyComponent={
              <View style={styles.emptyState}>
                <Text style={[styles.emptyTitle, { color: textMain }]}>
                  {selectedMood === 'all' && !searchText ? 'No Entries Yet' : 'No Matching Entries'}
                </Text>
                <Text style={[styles.emptyText, { color: textSub }]}>
                  {selectedMood === 'all' && !searchText 
                    ? 'Complete your first reflection to see it here!' 
                    : searchText
