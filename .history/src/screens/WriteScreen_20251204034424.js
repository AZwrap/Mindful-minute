...
import { useSharedPalette } from '../hooks/useSharedPalette'; // <--- FIXED: Added Import
...
export default function WriteScreen({ navigation, route }) {
  ...
  const palette = useSharedPalette(); // <--- FIXED: Defined palette
  ...
  async function playSound() {
    ...
    try {
      ...
      newSound.setOnPlaybackStatusUpdate((status) => {
        if (status.didJustFinish) { setIsPlaying(false); setSound(null); }
      });
    } catch (e) {
...
// ===== TIMER TICK (RESTORED SAFELY) =====
  const handleTick = (t) => {
    if (!mounted.current) return;

    // Use requestAnimationFrame for smoother UI updates if possible, 
    // but the key fix is ensuring setters are not causing infinite re-renders.
    
      setRemaining(t); // Update remaining time
      ...
      if (preserveTimerProgress) {
        ...
        // This setter is outside render and is acceptable in this callback
      ...
      if (t <= 0) {
        if (phase === 'writing') {
          ...
          if (currentCycle >= totalCycles) {
            ...
            if (soundEnabled || hapticsEnabled) playCompletionFeedback();
        ...
        // Immediately persist the final state after phase change
        if (preserveTimerProgress) {
            const pomodoroState = { phase, currentCycle, skipBreakAvailable, totalCycles };
            setDraftTimer(date, t, pomodoroState);
        }
      }
  };
  ...
  const skipBreak = () => {
    if (phase === 'break' && skipBreakAvailable) {
      ...
      if (nextCycle > totalCycles) {
        setRunning(false);
        setTimerCompleted(true);
        if (soundEnabled) playCompletionFeedback();
        Animated.timing(fade, { toValue: 0, duration: 600, useNativeDriver: true }).start();
      } else {
        setPhase('writing');
        setRemaining(writeDuration);
    ...
    >
      ...
      >
        <View style={styles.contentCard}>
          ...
          <Animated.View style={[{ opacity: fade }, styles.timerContainer]}>
            {showTimer && (
              <View style={styles.timerRow}>
                <Animated.View style={{ transform: [{ scale: resetAnim }], marginRight: 20 }}>
                  <PremiumPressable
                    onPress={handleReset}
                    haptic="light"
                    style={[styles.resetBtn, { backgroundColor: palette.card }]}
                  >
                ...
                <Animated.View
                  style={[
                    styles.timerPill,
                    {
                      ...
                      backgroundColor: phase === 'writing' ? palette.card : 'rgba(34,197,94,0.1)',
                      height: phase === 'break' && skipBreakAvailable ? 115 : 90,
                    },
                  ]}
                >
                  <View style={styles.phaseIndicator}>
                    <Text style={[styles.phaseText, { color: phase === 'writing' ? palette.accent : '#16A34A' }]}>
                      {phase === 'writing' ? '🖊️ Writing' : '⏸️ Break'}
                    </Text>
                    <Text style={[styles.cycleText, { color: palette.subtleText }]}>
                  ...
                  <Text style={[styles.timerStatus, { color: running ? palette.accent : palette.subtleText }]}>
                  ...
                  {phase === 'break' && skipBreakAvailable && (
                    <PremiumPressable
                      onPress={skipBreak}
                      haptic="light"
                      style={[styles.skipButton, { backgroundColor: palette.card }]}
                    >
          ...
          {!timerCompleted && (
            <PremiumPressable onPress={() => setShowTimer((s) => !s)} haptic="light" style={styles.toggle}>
              <Text style={{ color: palette.accent, fontWeight: '500' }}>
          ...
          {/* MAIN EDITOR */}
          <Animated.View
            style={[animatedInputStyle, { marginTop: timerCompleted ? -120 : 2, borderRadius: 14 }]}
            onLayout={(e) => setSectionPositions((prev) => ({ ...prev, editor: e.nativeEvent.layout.y }))}
          >
            <TextInput
              ref={inputRef}
              value={text}
                  ...
                  {suggestedMoods.map((suggestion) => (
                    ...
                    >
                      ...
                      {suggestion.confidence > 60 && (
                        <Text style={[styles.confidenceText, { color: palette.accent }]}>{suggestion.confidence}%</Text>
                      )}
                    </Pressable>
                  ))}
                </View>
          ...
          {(gratitudeModeEnabled || currentPrompt?.text?.toLowerCase().includes('grateful') || currentPrompt?.text?.toLowerCase().includes('thankful')) && (
            <View
              ...
              onLayout={(e) => setSectionPositions((prev) => ({ ...prev, gratitude: e.nativeEvent.layout.y }))}
            >
              <Pressable
                onPress={() => {
          ...
          <View style={{ flexDirection: 'row', gap: 8, marginTop: 16 }}>
            <PremiumPressable onPress={() => setRunning((r) => !r)} haptic="light" style={[styles.btnGhost, { borderColor: palette.border }]}>
              <Text style={[styles.btnGhostText, { color: palette.text }]}>{running ? 'Pause' : 'Resume'}</Text>
            </PremiumPressable>

            <PremiumPressable onPress={saveAndExit} disabled={!text.trim() && !audioUri} haptic="light" style={[styles.btnGhost, { borderColor: palette.accent, opacity: (!text.trim() && !audioUri) ? 0.5 : 1 }]}>
              <Text style={[styles.btnGhostText, { color: palette.accent }]}>Save & Exit</Text>
            </PremiumPressable>

            <PremiumPressable onPress={continueToMood} disabled={!text.trim() && !audioUri} haptic="light" style={[styles.btnPrimary, { backgroundColor: palette.accent, opacity: (!text.trim() && !audioUri) ? 0.5 : 1 }]}>
