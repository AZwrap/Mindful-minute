...
const continueToMood = async () => {
  ...
  // Check how many gratitude items are actually filled in
  const completedGratitudeItems = gratitudeEntries.filter(entry => entry?.trim()).length;

  // Treat either a gratitude-style prompt OR Gratitude Mode as eligible,
  // but only count it as a "gratitude entry" if at least 2 boxes are filled.
  const isGratitudePrompt =
  ...
  const isGratitudeEntry =
    (gratitudeModeEnabled || isGratitudePrompt) &&
    completedGratitudeItems >= 2;
