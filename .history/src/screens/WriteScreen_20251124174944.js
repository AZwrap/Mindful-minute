...
const continueToMood = async () => {
  ...
  // Check if this is a gratitude entry for bonus XP
  const isGratitudeEntry = (currentPrompt?.text?.toLowerCase().includes('grateful') || currentPrompt?.text?.toLowerCase().includes('thankful') || currentPrompt?.text?.toLowerCase().includes('appreciate'));
  const completedGratitudeItems = gratitudeEntries.filter(entry => entry?.trim()).length;
  const xpBonus = isGratitudeEntry && completedGratitudeItems >= 2 ? 10 : 0;

    // Track gratitude streak if this is a gratitude entry with at least 2 items
  if (isGratitudeEntry && completedGratitudeItems >= 2) {
    const { incrementGratitudeStreak } = useProgress.getState();
    incrementGratitudeStreak();
  }
  const achievementResult = applyDailySave({
  date,
  moodTagged: false, // Will be set in MoodTag screen
  wordCount: wordCount,
  usedTimer: running, // Based on your timer state
  entryHour: new Date().getHours(),
  isGratitudeEntry: isGratitudeEntry && completedGratitudeItems >= 2
});
