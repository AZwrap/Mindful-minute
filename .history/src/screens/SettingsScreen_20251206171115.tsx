...
{accentDropdownOpen && (
                      <View style={[styles.colorGrid, { backgroundColor: isDark ? "#1E293B" : "#F9FAFB", borderColor: palette.border, padding: 16 }]}>
                        <Text style={{ color: palette.sub, fontSize: 12, marginBottom: 8, fontWeight: '600' }}>Slide to Select Color</Text>
                        <View style={{ height: 40, borderRadius: 8, overflow: 'hidden', borderWidth: 1, borderColor: palette.border }}>
                          <Pressable
                            style={{ flex: 1 }}
                            onTouchStart={(e) => {
                              // Estimate width or use onLayout for precision (approx 300 for mobile padding)
                              const width = 300; 
                              const x = e.nativeEvent.locationX;
                              const hue = Math.max(0, Math.min(360, (x / width) * 360));
                              // Ensure hslToHex helper is defined at bottom of file
