      ...
      {prompt?.explanation && (
        <Text style={{ fontSize: 12, color: isDark ? '#94A3B8' : '#64748B', fontStyle: 'italic' }}>
          {'\n'}💡 {prompt.explanation}
            ...
            {/* Gratitude Journaling Section - Only shows for gratitude prompts */}
        {(currentPrompt?.text?.toLowerCase().includes('grateful') || currentPrompt?.text?.toLowerCase().includes('thankful') || currentPrompt?.text?.toLowerCase().includes('appreciate')) && (
          <View style={[
