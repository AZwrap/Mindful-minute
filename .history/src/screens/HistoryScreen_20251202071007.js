...
const SwipeableEntry = ({
  ...
  // ADD THIS: Get streak for the summary card
...
// FIXED: Select 'entries' directly (getters like 'map' are lost during storage save/load)
  ...
  const flashDate = useEntriesStore((s) => s.flashDate);
  const consumeFlashDate = useEntriesStore((s) => s.consumeFlashDate);
...
{/* 1. STATS SUMMARY WIDGET (Replaces Floating Button) */}
        <PremiumPressable
...
statsWidget: {
    ...
    borderRadius: 16,
    borderWidth: 1,
    marginBottom: 20, // Space between stats and filters
  },
  statsLabel: {
    fontSize: 11,
    fontWeight: '600',
