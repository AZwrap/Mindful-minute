...
// STOP TIMER when leaving screen (e.g. to MoodTag)
  useFocusEffect(
    React.useCallback(() => {
      setRunning(true); // Resume when coming back
      isScreenActive.current = true;
      
      return () => {
        setRunning(false); // PAUSE when leaving
        isScreenActive.current = false;
      };
    }, [])
  );
...
// FIXED: Navigate to MainTabs because 'Home' is now nested
