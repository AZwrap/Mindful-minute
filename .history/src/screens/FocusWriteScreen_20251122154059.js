...
export default function FocusWriteScreen() {
  ...
  // Get the correct function from your entries store
  const updateDraft = useEntries((s) => s.updateDraft);
  const saveDraft = useEntries((s) => s.saveDraft); // Try this if updateDraft doesn't exist
  const setDraft = useEntries((s) => s.setDraft); // Or this
  ...
  useEffect(() => {
    ...
    // Auto-save draft - try different function names
    if (date && text !== initialText) {
      try {
        // Try updateDraft first
        if (updateDraft) {
          updateDraft(date, text);
        } 
        // If that doesn't work, try saveDraft
        else if (saveDraft) {
          saveDraft(date, text);
        }
        // If that doesn't work, try setDraft
        else if (setDraft) {
          setDraft(date, text);
        }
        // Last resort - use the store directly
        else {
          const { updateDraft: ud } = useEntries.getState();
          if (ud) ud(date, text);
        }
      } catch (error) {
        console.log('Auto-save not available:', error);
      }
    }
  }, [text, date, initialText, updateDraft, saveDraft, setDraft]);

  // Handle save and exit
  const handleSaveAndExit = () => {
    // Try to save one last time before exiting
    if (date && text !== initialText) {
      try {
        if (updateDraft) {
          updateDraft(date, text);
        ...
        console.log('Final save failed:', error);
      }
    }
