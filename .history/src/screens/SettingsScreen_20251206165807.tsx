...
{accentDropdownOpen && (
                      <View style={[styles.colorGrid, { backgroundColor: isDark ? "#1E293B" : "#F9FAFB", borderColor: palette.border, flexDirection: 'column', gap: 12 }]}>
                        {/* Preset Grid */}
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
                          {APP_COLORS.map((color) => (
                            <Pressable
                              ...
                              style={[styles.colorDot, { backgroundColor: color, borderColor: accentColor === color ? palette.text : 'transparent' }]}
                            >
...
{/* Custom Gradient Picker */}
                        <View style={{ paddingTop: 12, borderTopWidth: 1, borderTopColor: palette.border }}>
                          <Text style={{ color: palette.sub, fontSize: 12, marginBottom: 8, fontWeight: '600' }}>Custom Spectrum</Text>
                          <View style={{ height: 40, borderRadius: 8, overflow: 'hidden', borderWidth: 1, borderColor: palette.border }}>
                            <Pressable
                              ...
                              onTouchStart={(e) => {
                                ...
                                const width = 300; // Approx width of container
                                const x = e.nativeEvent.locationX;
                                const hue = Math.max(0, Math.min(360, (x / width) * 360));
                                const color = hslToHex(hue, 100, 50); // 100% Saturation, 50% Lightness
                                setAccentColor(color);
                                setCustomHex(color);
                                if (hapticsEnabled) Haptics.selectionAsync();
                              }}
                              onLayout={(e) => {
                                // Optional: You could save e.nativeEvent.layout.width to state for precision
                              }}
                            >
                              <LinearGradient
                                colors={[
                          ...
                          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 8 }}>
                             ...
                             <Text style={{ color: palette.text, fontWeight: '600' }}>{accentColor}</Text>
                          </View>
                        </View>
                      </View>
                    )}
