...
export default function WriteScreen({ navigation, route }) {
  ...
  const [wordCount, setWordCount] = useState(
    () => (route.params?.text || '').trim().split(/\s+/).filter((w) => w.length > 0).length
  );
  ...
  // ===== CRITICAL FIX: SAFE TIMER TICK =====
  // Uses requestAnimationFrame to prevent updates during render cycle
  const handleTick = (t) => {
  ...
  const saveAndExit = () => {
    ...
    upsert({ date, text: text.trim(), prompt: { text: prompt?.text }, createdAt: new Date().toISOString(), isComplete: false, audioUri });
    if (!preserveTimerProgress) setDraftTimer(date, writeDuration, null);
  ...
  const continueToMood = async () => {
    ...
    if (isGratitudeEntry) useProgress.getState().incrementGratitudeStreak();

    upsert({
      date, text: text.trim(), prompt: { text: prompt?.text }, createdAt: new Date().toISOString(),
      isComplete: false, isGratitude: isGratitudeEntry, gratitudeItems: gratitudeEntries.filter(e => e?.trim()), xpBonus, audioUri
    });

    if (xpBonus > 0) showToast('+10 XP Gratitude Bonus!');
    setTimeout(() => {
      navigation.navigate('MoodTag', { date, prompt, text, suggestedMood: selectedSuggestedMood, isGratitudeEntry, xpBonus, audioUri });
    }, 50);
  };
  ...
  const handleTextChange = (newText) => {    
    ...
    if (newText.length > 15) setSuggestedMoods(getMoodSuggestions(newText));
    else setSuggestedMoods([]);
  };
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        <ScrollView ref={scrollRef} style={styles.container} contentContainerStyle={[styles.scrollContent, { paddingBottom: 20 + keyboardHeight }]} keyboardShouldPersistTaps="handled">
          <View style={styles.contentCard}>
            ...
            {!timerCompleted && (
              <PremiumPressable onPress={() => setShowTimer((s) => !s)} style={styles.toggle}>
            ...
            // ===== CRITICAL FIX: SAFE LAYOUT EXTRACTION =====
            // Extracts the value first to avoid accessing it in the callback
            onLayout={(e) => {
              const layoutY = e.nativeEvent.layout.y;
              setSectionPositions((prev) => ({ ...prev, editor: layoutY }));
            }}>
              <TextInput
                ref={inputRef}
                value={text}
                ...
                <View style={styles.suggestionsRow}>
                  {suggestedMoods.map((suggestion) => (
                    <Pressable
                      ...
                      style={[styles.suggestionChip, { backgroundColor: selectedSuggestedMood === suggestion.mood ? palette.accent + '40' : palette.card, borderColor: selectedSuggestedMood === suggestion.mood ? palette.accent : palette.border, borderWidth: selectedSuggestedMood === suggestion.mood ? 2 : 1 }]}
                    >
                      <Text style={[styles.suggestionText, { color: palette.accent, fontWeight: '700' }]}>{suggestion.mood}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
          ...
          {(gratitudeModeEnabled || currentPrompt?.text?.toLowerCase().includes('grateful') || currentPrompt?.text?.toLowerCase().includes('thankful')) && (
            <View
              ...
              onLayout={(e) => {
                // Extract synchronously to prevent crash
                const layoutY = e.nativeEvent.layout.y;
