import * as FileSystem from 'expo-file-system';
import * as Sharing from 'expo-sharing';
import * as Print from 'expo-print';

export const exportSingleEntry = async (entry, format = 'txt') => {
  try {
    let fileUri = '';
    let mimeType = '';
    let fileName = `MindfulMinute_${entry.date}`;

    if (format === 'pdf') {
      // 🟢 PDF Generation
      fileName += '.pdf';
      mimeType = 'application/pdf';
      
      const htmlContent = `
        <!DOCTYPE html>
        <html>
          <head>
            <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=no" />
            <style>
              @page { margin: 40px; }
              body { font-family: 'Helvetica', sans-serif; padding: 20px; color: #1F2937; }
              h1 { color: #6366F1; border-bottom: 2px solid #E5E7EB; padding-bottom: 16px; margin-bottom: 24px; font-size: 24px; }
              .meta { margin-bottom: 30px; color: #6B7280; font-size: 14px; }
              .label { font-weight: bold; color: #374151; margin-right: 8px; }
              .prompt-box { background-color: #F3F4F6; padding: 16px; border-left: 4px solid #6366F1; border-radius: 8px; margin-bottom: 24px; }
              .prompt-text { font-weight: 600; font-size: 16px; color: #111827; margin: 0; }
              .content { font-size: 16px; line-height: 1.6; white-space: pre-wrap; margin-bottom: 40px; }
              .footer { text-align: center; font-size: 10px; color: #9CA3AF; margin-top: 40px; border-top: 1px solid #F3F4F6; padding-top: 20px; }
              .mood-badge { display: inline-block; background-color: #EEF2FF; color: #6366F1; padding: 4px 12px; border-radius: 99px; font-weight: bold; font-size: 12px; }
            </style>
          </head>
          <body>
            <h1>Mindful Minute Entry</h1>
            
            <div class="meta">
              <p><span class="label">Date:</span> ${new Date(entry.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
              <p><span class="label">Time:</span> ${new Date(entry.createdAt || new Date()).toLocaleTimeString()}</p>
              <p><span class="label">Mood:</span> <span class="mood-badge">${entry.moodTag?.value || 'Not tracked'}</span></p>
            </div>

            <div class="prompt-box">
              <p class="prompt-text">${entry.prompt?.text || entry.promptText || "Freestyle Entry"}</p>
            </div>

            <div class="content">
              ${entry.text}
            </div>
            
            <div class="footer">
              Generated by Mindful Minute App • Keep growing 🌱
            </div>
          </body>
        </html>
      `;

      const { uri } = await Print.printToFileAsync({ html: htmlContent });
      fileUri = uri;

    } else if (format === 'json') {
      // 🟢 JSON Export
      fileName += '.json';
      mimeType = 'application/json';
      const jsonContent = JSON.stringify(entry, null, 2);
      fileUri = FileSystem.documentDirectory + fileName;
      await FileSystem.writeAsStringAsync(fileUri, jsonContent);

    } else if (format === 'csv') {
      // 🟢 CSV Export
      fileName += '.csv';
      mimeType = 'text/csv';
      // Simple CSV escaping
      const cleanText = (entry.text || '').replace(/"/g, '""'); 
      const cleanPrompt = (entry.prompt?.text || '').replace(/"/g, '""');
      const csvContent = `Date,Prompt,Mood,Text\n"${entry.date}","${cleanPrompt}","${entry.moodTag?.value || ''}","${cleanText}"`;
      fileUri = FileSystem.documentDirectory + fileName;
      await FileSystem.writeAsStringAsync(fileUri, csvContent);

    } else {
      // 🟢 Default Text Export
      fileName += '.txt';
      mimeType = 'text/plain';
      const textContent = `Date: ${entry.date}\nPrompt: ${entry.prompt?.text || 'None'}\nMood: ${entry.moodTag?.value || 'None'}\n\n${entry.text}`;
      fileUri = FileSystem.documentDirectory + fileName;
      await FileSystem.writeAsStringAsync(fileUri, textContent);
    }

    // 🟢 Share the generated file
    if (await Sharing.isAvailableAsync()) {
      await Sharing.shareAsync(fileUri, {
        mimeType: mimeType,
        dialogTitle: `Export Entry as ${format.toUpperCase()}`,
        UTI: mimeType // Helpful for iOS to know which apps can open it
      });
    } else {
      alert("Sharing is not available on this device");
    }

  } catch (error) {
    console.error("Export failed:", error);
    alert("Failed to export file.");
  }
};