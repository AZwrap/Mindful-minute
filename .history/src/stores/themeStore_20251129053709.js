...
// If dynamic theme has never been computed, compute it now
if (theme === "dynamic" && currentDynamicTheme === null) {
  updateDynamicThemeAndScheduleNext(get, get);
}
...
useTheme.getState().loadTheme().then(() => {
  ...
  if (theme === "dynamic") {
    updateDynamicThemeAndScheduleNext(
      useTheme.setState ? useTheme.setState : useTheme.getState,
      useTheme.getState
    );
  }
});
