...
  useColorScheme, 
  Animated,
  AccessibilityInfo, // ADD THIS
} from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import * as Haptics from 'expo-haptics'; // ← ADD THIS
import { useFocusEffect, useNavigation, useRoute } from '@react-navigation/native';
import { todayISO, promptOfDay, POSITIVE_PROMPTS, NEUTRAL_PROMPTS, CHALLENGING_PROMPTS, ALL_PROMPTS } from '../lib/prompts';
...
import PremiumPressable from '../components/PremiumPressable'; // ← ADD THIS
...
  const [isScreenReaderEnabled, setIsScreenReaderEnabled] = useState(false); // ADD THIS
...
      const entries = useEntries.getState().listDesc();
      const recentPrompts = entries.slice(0, 30).map(e => e.promptText);

      // Helper to get a random, unused prompt from a category
      const getUnusedPrompt = (category) => {
        const unused = category.filter(p => !recentPrompts.includes(p));
        if (unused.length > 0) {
...
      if (entries.length > 2) {
        const lastThreeMoods = entries.slice(0, 3).map(e => e.moodTag.value);
        const moodCategories = getMoodCategories();

        const lastMoodCategory = moodCategories[lastThreeMoods[0]];

        let promptText = null;

        // If the last mood was positive, suggest a positive prompt.
        if (lastMoodCategory === 'Positive') {
          promptText = getUnusedPrompt(POSITIVE_PROMPTS);
        }
        // If the last 2 moods were challenging, suggest a challenging prompt.
        else if (
          lastMoodCategory === 'Challenging' &&
          moodCategories[lastThreeMoods[1]] === 'Challenging'
        ) {
          promptText = getUnusedPrompt(CHALLENGING_PROMPTS);
        }
        
        // Fallback to a neutral or any other prompt
        if (!promptText) {
          promptText = getUnusedPrompt(NEUTRAL_PROMPTS) ?? getUnusedPrompt(ALL_PROMPTS);
        }

        if (promptText) {
          setToday({ id: 'smart', text: promptText, isCustom: false });
          return;
        }
      }
      
      // Default fallback if no smart prompt is generated
